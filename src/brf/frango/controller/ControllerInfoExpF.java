...
import brf.frango.view.ViewInfoExpF;
...
public class ControllerInfoExpF extends KeyAdapter implements ActionListener, FocusListener, ItemListener {

	private final ControllerF controller;
	private ViewInfoExpF viewInfoExp;
	...
	private InfoExpVOF info1, info2;
	private ViewInfoExpF view1, view2;
	private boolean duplaDigitacao;
	private Border defaultJTF, defaultJCB, defaultJP;

	public ControllerInfoExpF(ControllerF c) {
		controller = c;
		duplaDigitacao = false;
	}

	public void openWindow(String idDigitador) {
		viewInfoExp = new ViewInfoExpF();
		viewInfoExp.setTitle("DIGEX - Frango");
		...
		// order.add(viewInfoExp.getTesteJFT()); //removido para inserir automaticamente
	...
	public void keyPressed(KeyEvent e) {
		...
		if (e.getKeyCode() == KeyEvent.VK_LEFT && !e.getSource().equals(viewInfoExp.getProtocoloJFT())) {
			...
			if (src instanceof JFormattedTextField) {
				...
				Component prev = viewInfoExp.getFocusTraversalPolicy().getComponentBefore(viewInfoExp,
				...
				if (prev instanceof JFormattedTextField) {
					if (prev.equals(viewInfoExp.getIdadeFaseJFT()) || prev.equals(viewInfoExp.getNrFasesJFT())) {
						...
						faseCounter = 1;
							...
							|| prev.equals(viewInfoExp.getAleijado7JFT())
							|| prev.equals(viewInfoExp.getAleijado8JFT())) {
	...
	public void keyTyped(KeyEvent e) {
		try {
			if (e.getSource() == viewInfoExp.getOpcaoJFT()) {
				...
				switch (e.getKeyChar()) {
				case KeyEvent.VK_0:
					int n = JOptionPane.showConfirmDialog(viewInfoExp, "Deseja realmente sair do programa?",
				...
				case KeyEvent.VK_1:
					if (!duplaDigitacao) {
						info1 = new InfoExpVOF(idDigitador, protocolo, idLocal, local, aviario, teste,
								Integer.parseInt(viewInfoExp.getNrCamaJFT().getText()), idArea, area,
								...
								Integer.parseInt(viewInfoExp.getNrFasesJFT().getText()), idadeFase,
								Integer.parseInt(viewInfoExp.getNrAbatesJFT().getText()), dataAbate,
						...
						info2 = new InfoExpVOF(idDigitador, protocolo, idLocal, local, aviario, teste,
								Integer.parseInt(viewInfoExp.getNrCamaJFT().getText()), idArea, area,
								Integer.parseInt(viewInfoExp.getNrBaiaJFT().getText()), inicioExp, fimExp,
								Integer.parseInt(viewInfoExp.getIdadeAlojJFT().getText()),
								Integer.parseInt(viewInfoExp.getEqualizacaoJFT().getText()),
			...
			JOptionPane.showMessageDialog(null, "Verifique os dados digitados", "Dados digitados com Zero",
					JOptionPane.WARNING_MESSAGE);
		}

		if (e.getKeyChar() == KeyEvent.VK_ESCAPE) {
			int option = JOptionPane.showConfirmDialog(viewInfoExp, "Deseja realmente voltar para tela anterior?",
			...
			if (src == viewInfoExp.getProtocoloJFT()) {
				...
				TextFormatter.formatStringJFT((JFormattedTextField) src, text, 10);
				VerifyRules.verifyData(viewInfoExp.getDataAbateJFT().getText());
	...
	public void mostraCamposErrados() {
		...
		if (info2.getProblemas()[6] || info2.getProblemas()[7]) {
		...
		// viewInfoExp.getRetiradaJP().setBorder(BorderFactory.createLineBorder(Color.RED,
		// 2));
		// break;
		// }
		// }
